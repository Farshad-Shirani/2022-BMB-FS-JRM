%----------------------------------------------------------------------------------------
% Article: 	“Competition, Trait Variance Dynamics, and the Evolution of a Species' Range”
% Journal: 	Bulletin of Mathematical Biology
% Authors:	Farshad Shirani, Georgia Institute of Technology (f.shirani@gatech.edu)
%           Judith R. Miller, Georgetown University (Judith.Miller@georgetown.edu)
%----------------------------------------------------------------------------------------
...
for i = 1 : numPopulations
    populations(i).density = zeros(I, J-1, length(storageTimes)); 
    populations(i).trait_mean = zeros(I, J-1, length(storageTimes)); 
    populations(i).trait_variance = zeros(I, J-1, length(storageTimes));
...
variableLocation = zeros(systemSize, I*(J-1));
for i = 1:systemSize
    variableLocation(i,:) = [1 : systemSize : systemSize * I * (J-1)] + (i-1);
end

solution = zeros(I, J-1, systemSize);
...
solution_star = zeros(I, J-1, systemSize);
...
for k = 1 : length(t)-1
    ...
    surf(fig4, X1, X2, solution(:,:,4)');
    surf(fig5, X1, X2, solution(:,:,5)');
    surf(fig6, X1, X2, solution(:,:,6)');
    drawnow
    %tic
    ...
    for i = 1 : systemSize
        solution_star(:,:,i) =  reshape( U_star(variableLocation(i,:)), I, J-1);
    end      
    ...
    for i = 1 : systemSize
        solution(:,:,i) =  reshape( U(variableLocation(i,:)), I, J-1);
    end
...
% save('Results\sol_nominal.mat', 'populations', 'modelParameters', 'simulationParameters', 'discretizationParamaters');
